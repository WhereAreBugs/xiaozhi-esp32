...
// Forward declaration from esp_blufi.c (internal API)
...
#ifdef CONFIG_BT_BLUEDROID_ENABLED
extern "C" void esp_blufi_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param);
#endif
...
#ifdef CONFIG_BT_BLUEDROID_ENABLED
esp_err_t Blufi::_host_init(void) {
...
// --- Private Implementation (from blufi.cpp application logic) ---
...
void Blufi::_handle_event(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param) {
     switch (event) {
        ...
        case ESP_BLUFI_EVENT_RECV_STA_PASSWD:
            ...
            break;
        // Handle other cases like SOFTAP config if needed...
        default:
...
uint16_t Blufi::_checksum_func_trampoline(uint8_t iv8, uint8_t *data, int len) {
    return GetInstance()._crc_checksum(iv8, data, len);
}