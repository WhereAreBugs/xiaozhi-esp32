...
private:
    ...
    // Initialization logic from blufi_init.c
    ...
    // Security logic from blufi_security.c
    void _security_init();
    ...
    uint16_t _crc_checksum(uint8_t iv8, uint8_t *data, int len);

    // Application logic from blufi.cpp
    void _handle_event(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param);
    int _get_softap_conn_num();
...
#define PSK_LEN                 16
    ...
    // State variables from blufi.cpp
